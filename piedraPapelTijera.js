...
// Aquí configuro los botones
document.getElementsByTagName("button")[0].addEventListener("click", usuarioIntroducido, false);
...
function comprobarNombreJugador(nombreAComprobarDelJugador) {
	if ((nombreAComprobarDelJugador.length > 3) && (isNaN(nombreAComprobarDelJugador[0]))) {
		return true;
	} else {
		return false;
	}
...
function seleccionaTiradaJugador(e) {
	...
	// Aquí cambio la apariencia al resto de elementos que no sean el que se pulsan.
...
function reset() {
	...
	opciones[opciones.length-1].src = creaRutaImg("", "defecto");;
